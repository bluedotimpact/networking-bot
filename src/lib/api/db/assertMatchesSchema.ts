import { matchesType, TsTypeString } from './mapping/types';
...
export function assertMatchesSchema<T extends Item>(
  ...
  if (typeof data !== 'object' || data === null) {
    throw new Error(`Item for ${table.name} is not an object`);
  }
  ...
  (Object.entries(table.schema) as ([keyof T & string, TsTypeString])[]).forEach(([fieldName, type]) => {
    ...
    if (value === undefined) {
      ...
      throw new Error(`Item for ${table.name} table is missing field '${fieldName}' (expected ${type})`);
    }

    if (!matchesType(value, type)) {
      throw new Error(`Item for ${table.name} table has invalid value for field '${fieldName}' (actual type ${typeof value}, but expected ${type})`);
    }
  });
}
