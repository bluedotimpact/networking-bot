    ...
    schemaEntries.map(([outputFieldName, tsType]) => {
      ...
      if (Array.isArray(mappingToAirtable)) {
        if (value === null) {
          ...
          throw new Error(`Expected field ${table.name}.${outputFieldName} to match type \`${tsType}\` but got null. This should never happen in normal operation as it should be caught before this point.`);
        }
